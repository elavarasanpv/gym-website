...
import ContactUsPageGraphic from '@/assets/pexels-shkrabaanthony-4662341.jpg';
          ...
          <motion.div className='mt-10 basis-3/5 md:mt-0'
           initial="hidden" 
                ...
                variants={{hidden: {opacity: 0, y: 50}, visible: {opacity: 1, y: 0}}}>
                    ...
                    action="https://formsubmit.co/4c42bb282790e8068dfb4945be502982" method='POST'>
                     <input className={inputStyles} type='text' placeholder='NAME' {...register('name',{
                        required: true,
                        maxLength:100
                     })}/>
                        {errors.name && <p className='mt-1 text-primary-500'> {errors.name.type === 'required' && "This field is required."} 
                        ...
                        {errors.email && <p className='mt-1 text-primary-500'> {errors.email.type === 'required' && "This field is required."} 
                        {errors.email.type === "pattern" && "Invalid email address."}</p>}

                         <textarea className={inputStyles}  placeholder="MESSAGE"
                          rows={4}
                          cols={50}  {...register('message',{
                        required: true,
                        maxLength:2000
                     })}/>
                        {errors.message && <p className='mt-1 text-primary-500'> {errors.message.type === 'required' && "This field is required."} 
